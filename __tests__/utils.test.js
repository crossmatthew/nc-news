...
describe('checkValueExists', () => {
  test('should return an empty array if article_id exists, but has no comments on the comments table', () => {
    return checkValueExists('comments', 'article_id', 6)
    .then((result) => {
      expect(result).toMatchObject([])
    })
  });
  test('should return a rejected promise when passed a non-existent article_id', () => {
    return expect(checkValueExists('articles', 'article_id', 9000000)).rejects.toEqual(undefined)
    })
  });
